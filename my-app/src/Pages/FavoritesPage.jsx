...
export function FavoritesPage() {
  ...
  return (
    ...
    <div className="flex gap-4 mb-4">
      ...
      Object.keys(groupedFavorites).map((showId) => (          <div key={showId} className="mb-6">            <h2 className="text-xl font-bold mb-4">{groupedFavorites[showId][0].showTitle}</h2>            <ul>              {groupedFavorites[showId].map((fav) => (                <li                  key={fav.id}                  className="flex justify-between items-center bg-gray-800 p-4 rounded-lg shadow-lg"                >                  <span>                    {fav.episodeNumber}. {fav.title}                  </span>                  <button                    onClick={() => removeFromFavorites(fav.id)}                    className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600"                  >                    Remove                  </button>                </li>              ))}            </ul>          </div>        ))
       )}  
         </div>
           );
